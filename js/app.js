/**
 * SmartLink - Application JavaScript Principal
 * Version: 1.0.0
 */

const CONFIG = {
    MAX_LINKS: 50, // Limite technique raisonnable
...
class SmartLinkApp {
    ...
    saveProfile() {
        try {
            ...
            this.saveUserData();
            this.renderProfile();
            this.showNotification('Profil sauvegardé !', 'success');
        } catch (error) {
    ...
    showAddLinkModal() {
        if (!this.canAddMoreLinks()) {
            this.showNotification(`Limite technique de ${CONFIG.MAX_LINKS} liens atteinte !`, 'warning');
            return;
        }
    ...
    deleteLink(index) {
        if (confirm('Supprimer ce lien ?')) {
            ...
            this.saveUserData();
            this.renderLinks();
            this.showNotification('Lien supprimé', 'success');
        }
    }
    ...
    saveLinkChanges() {
        ...
        if (!title || !url) {
            this.showNotification('Titre et URL requis', 'error');
            return;
        }
        ...
        const modal = bootstrap.Modal.getInstance(document.getElementById('linkEditModal'));
        modal.hide();
        this.showNotification('Lien sauvegardé', 'success');
    }
        ...
        try {
            ...
            if (!stats.links[linkId]) {
                stats.links[linkId] = {
                    clicks: 0,
                    lastClick: null,
                    title: this.currentUser.links[linkIndex]?.title || 'Lien supprimé'
                };
            }
            ...
            console.error('Erreur suivi clics:', error);
        }
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (downloadQRBtn) {
        console.log('Bouton téléchargement QR trouvé, ajout de l\'événement...');
        downloadQRBtn.addEventListener('click', (e) => {
            e.preventDefault();
            console.log('Clic sur le bouton téléchargement QR détecté');
            downloadQRCode();
        });
    } else {
        console.error('Bouton téléchargement QR non trouvé');
    }
    ...
    if (qrToggleBtn) {
        qrToggleBtn.addEventListener('click', () => {
            ...
            setTimeout(() => {
                generateQRCode();
            }, 300); // Délai pour que le modal soit visible
        });
    }

    // Générer QR Code quand le modal Bootstrap s'ouvre
    ...
    // Animations au scroll pour la page d'accueil
    if (window.location.pathname.includes('index.html') || window.location.pathname === '/') {
        initScrollAnimations();
        initCounterAnimations();
        // initTypingEffect(); // Désactivé pour éviter les problèmes d'affichage
    }
});
...
function generateQRCode() {
    ...
    if (!qrContainer) {
        console.error('Container QR Code non trouvé');
        return;
    }
    ...
    if (typeof QRCode === 'undefined') {
        qrContainer.innerHTML = '<p class="text-muted">Bibliothèque QR Code non chargée</p>';
        return;
    }
    ...
    try {
        ...
        qrContainer.innerHTML = '<p class="text-muted">Erreur lors de la génération</p>';
    }
}

function downloadQRCode() {
    console.log('Tentative de téléchargement QR Code...');
    
    const canvas = document.querySelector('#qrcode canvas');
    if (canvas) {
        try {
            console.log('Canvas trouvé, création du lien de téléchargement...');
            ...
            if (link.href.startsWith('data:')) {
                ...
                // Notification simple sans dépendance
        ...
        console.error('Aucun canvas QR Code trouvé');
        alert('Aucun QR Code à télécharger. Veuillez d\'abord générer le QR Code.');
    }
}
